<?php
include('admin_sidebar.php');

  ?>
   <div class="content-wrapper">
    <div class="container-fluid">
      <!-- Breadcrumbs-->

      <div class="row">
        <div class="col-12">
        <div class="card mb-3">
        <div class="card-header">
            <i class="fa fa-table"></i>&nbsp; Withdraw History</div>
        <div class="card-body">
            <div class="table-responsive" id="texthint">
             <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                       
                         <?php if(!empty($result)){
                              ?>
                  <div class="row margin-bottom">
                       <div class="col-md-11 offset-md-1">
                           <form method="post" action="<?php echo base_url(); ?>Account_Controller/Search_Withdraw_History" enctype="multipart/form-data">
                       <div class="form-group">
                            <div class="form-row">
                            <div class="col-md-4">                              
                                <div class="dat-picker">
                                    <span><label>From</label> </span>
                                    <input id="datepicker" name="SearchDateFrom" class="form-control" type="date" > 
                                </div>
                           </div>
                          <div class="col-md-3">
                              <div class="dat-picker">
                                  <span>
                                      <label>To</label> </span>
                                      <input id="datepicker" name="SearchDateTo" class="form-control" type="date"  >
                                 
                              </div>
                                                                              
                     </div>
                      
                      <div class="col-md-3" >
                         
                            <input id="search" name="SearchItem" class="form-control" type="text" placeholder="Text">
                           
                        
                      </div>
                         <div class="col-md-1">
                                               <?php
                    $data=$this->input->get('acc_no');
                    if(!empty($data))
                    {
                        ?>
                  <input type="hidden" name="AccountNumber" value="<?php echo $data; ?>">
  <?php                      
                    }
?>
                             <button class="btn btn-outline-info" name="Search"><i class="fa fa-search"></i>&nbsp;Search</button>
                         </div>
                     </div>
                  </div>
                           
                           </form>    
</form>
                       </div>
                   </div>
                      <thead >
                            <tr>
                              <th scope="col">#</th>
                              <th scope="col">Withdraw Date</th>
                              <th scope="col">Withdraw Amount</th>
                              <th scope="col">Withdraw Type</th>
                              <th scope="col">Withdraw Reason</th>
                                <th scope="col">Account Number</th>
                            </tr>
                       </thead>

                      <tbody>
                        <?php

                        $i=0;

                        foreach($result as $results){
                            $i=$i+1;
                            ?>
                        <tr>
                            <td><?php echo $i;?></td>
                            <td><?php echo $results->WithdrawDate;?></td>
                            <td><?php echo number_format($results->WithdrawAmount);?></td>
                            <td><?php echo $results->WithdrawType;?></td>
                            <td><?php echo $results->WithdrawReason;?></td>
                            <td><?php echo $results->AccountNumber;?></td>
                             </tr>
                          <?php }
                         }
                          else {
                              ?>
                             <tr>
                                 <td>No Data Is Available</td>
                             </tr>
                             <?php
     
 }
                       ?>
                       
                      <?php  

                      
                      
                      if(!empty($search))
                       {
                          $value=$this->input->get('acc_no');
                          if(!empty($value))
                          {
                           ?>
                        <tr> <td colspan="3">
                                <a href="<?php echo base_url();?>/Account_Controller/Get_Specific_Acc_Withdraw_History?acc_no=<?php echo $value;?>"> << Back</a>
                            </td></tr>
                               <?php
                       }
                       else{
                          ?>
                         <tr> <td colspan="3">
                                <a href="<?php echo base_url();?>/Account_Controller/Get_Mother_Acc_Withdraw_History"> << Back</a>
                            </td></tr>
                        <?php
                       }
                          }
                         ?>
                        <tr >
                            <td colspan="3">
                                <?php echo $this->pagination->create_links();?>
                            </td>
                        </tr>
                  </tbody>

             </table>

         </div>
        </div>
      </div>
    </div>
      </div>
    </div>
   </div>


<?php

    include('logout_footer.php');

?>
